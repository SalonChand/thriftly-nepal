...
const db = mysql.createPool({
    host: 'localhost', 
    user: 'root', 
    password: '', // ⚠️ CHECK YOUR PASSWORD
...
app.get('/my-stats/:id', verifyUser, (req, res) => {
    ...
    // Safe queries using COALESCE for empty data
    const q1 = `SELECT COALESCE(SUM(p.price), 0) as e, COUNT(o.id) as s FROM orders o JOIN products p ON o.product_id = p.id WHERE o.seller_id = ?`;
    const q2 = `SELECT COALESCE(SUM(views), 0) as v, COUNT(id) as a FROM products WHERE seller_id = ?`;
    const q3 = `SELECT p.title, p.price, o.order_date FROM orders o JOIN products p ON o.product_id = p.id WHERE o.seller_id = ? ORDER BY o.order_date DESC LIMIT 5`;

    db.query(q1, [u], (err, r1) => {
        if(err) { return res.json({ earnings: 0, sold: 0, views: 0, active: 0, recent: [] }); }
        db.query(q2, [u], (err2, r2) => {
            db.query(q3, [u], (err3, r3) => {
                res.json({
                    earnings: r1[0]?.e || 0,
                    sold: r1[0]?.s || 0,
                    views: r2[0]?.v || 0,
...
app.get('/my-conversations/:id', verifyUser, (req, res) => {
    ...
    db.query(sql, [userId, userId, userId], (err, data) => {
        ...
        // Fetch usernames
        const promises = data.map(conv => {
            return new Promise((resolve) => {
                db.query("SELECT username FROM users WHERE id = ?", [conv.other_user_id], (e, u) => {
                    conv.other_user_name = u[0]?.username || "User";
                    resolve(conv);
                });
            });
        });
        Promise.all(promises).then(completedData => res.json(completedData));
    });
});
...
app.get('/my-offers/:id', verifyUser, (req, res) => {
    const userId = req.params.id;
    const sqlRec = `SELECT o.*, p.title, p.image_url, u.username as buyer_name FROM offers o JOIN products p ON o.product_id = p.id JOIN users u ON o.buyer_id = u.id WHERE o.seller_id = ? ORDER BY o.created_at DESC`;
    const sqlSent = `SELECT o.*, p.title, p.image_url, u.username as seller_name FROM offers o JOIN products p ON o.product_id = p.id JOIN users u ON o.seller_id = u.id WHERE o.buyer_id = ? ORDER BY o.created_at DESC`;
    
    db.query(sqlRec, [userId], (err, rec) => {
        if(err) console.log("Rec Error:", err);
        db.query(sqlSent, [userId], (err2, sent) => {
            if(err2) console.log("Sent Error:", err2);
            res.json({ received: rec || [], sent: sent || [] });
        });
    });
});
...
app.post('/offers', verifyUser, (req, res) => {
    ...
    db.query("INSERT INTO offers (`product_id`, `buyer_id`, `seller_id`, `offer_amount`) VALUES (?)", [[req.body.product_id, req.user_id, req.body.seller_id, req.body.offer_amount]], () => {
        createNotification(req.body.seller_id, 'offer', 'New offer received');
        res.json({Status: "Success"});
    });
});
app.put('/offers/:offerId', verifyUser, (req, res) => {
    db.query("UPDATE offers SET status = ? WHERE id = ?", [req.body.status, req.params.offerId], () => {
        db.query("SELECT buyer_id FROM offers WHERE id = ?", [req.params.offerId], (e, d) => {
            if(d.length > 0) createNotification(d[0].buyer_id, 'offer', `Your offer was ${req.body.status}`);
        });
        res.json({Status: "Success"});
    });
});
...
app.get('/stories', (req, res) => {
    // 🛡️ FIXED: Using LEFT JOIN so stories show even if user is missing
    const sql = `SELECT s.*, COALESCE(u.username, 'Unknown') as username, u.profile_pic, (SELECT COUNT(*) FROM story_comments WHERE story_id = s.id) as comment_count FROM stories s LEFT JOIN users u ON s.user_id = u.id ORDER BY s.created_at DESC`;
    db.query(sql, (err, data) => res.json(data || []));
});
app.put('/stories/like/:id', (req, res) => {
    db.query("UPDATE stories SET likes = likes + 1 WHERE id = ?", [req.params.id], () => res.json({ Status: "Success" }));
});
...
app.post('/stories/:id/comment', verifyUser, (req, res) => {
    ...
    db.query("INSERT INTO story_comments (story_id, user_id, comment, parent_id) VALUES (?)", [[req.params.id, req.user_id, comment, parent_id || null]], () => {
...
app.post('/stories/:id/report', verifyUser, (req, res) => {
    db.query("INSERT INTO story_reports (reporter_id, story_id, reason) VALUES (?)", [[req.user_id, req.params.id, req.body.reason]], () => { createNotification(1, 'admin', `New Story Report`); res.json({ Status: "Success" }); });
});
