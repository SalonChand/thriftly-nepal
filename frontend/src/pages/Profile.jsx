...
import { ShoppingBag, CheckCircle, Trash2, Heart, Edit2, Camera, X, Star, Package, Truck, Rocket, Gavel, MessageSquare } from 'lucide-react';
...
const Profile = () => {
    ...
    const [conversations, setConversations] = useState([]); // 🆕 Messages Data
    ...
    // 🆕 CHAT STATES
    const [activeChat, setActiveChat] = useState(null); // { sellerId, productId }
    ...
    const handleBoost = (productId) => {
        ...
        const params = { 
            amount: boostPrice, tax_amount: "0", total_amount: boostPrice, 
            transaction_uuid: transactionUuid, product_code: productCode, 
            product_service_charge: "0", product_delivery_charge: "0", 
            success_url: `http://localhost:5173/payment-success?pid=${productId}&type=boost`, 
    ...
    return (
        <div className="min-h-screen bg-[#FDFBF7] p-6 font-sans pt-24">
            ...
            {/* CHAT WINDOW (Opens when clicking Reply) */}
            {activeChat && (
                <Chat 
                    currentUser={user} 
                    sellerId={activeChat.other_user_id} 
                            ...
                            {conversations.map((conv, idx) => (
                                <div key={idx} className="bg-white p-4 rounded-xl border border-stone-200 flex justify-between items-center hover:shadow-md transition">
                                    ...
                                    <button 
                                        ...
                                        className="bg-stone-900 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-orange-600"
                                    >
                                        Reply
                                    </button>
                                </div>
