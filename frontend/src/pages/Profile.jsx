...
import { ShoppingBag, CheckCircle, Trash2, Heart, Edit2, Camera, X, Star, Package, Truck, Rocket, Gavel, MessageSquare, Clock, BarChart3 } from 'lucide-react';
...
const Profile = () => {
    ...
    const [stats, setStats] = useState({ earnings: 0, sold: 0, views: 0, active: 0, recent: [] });
    ...
    const refreshData = () => {
        ...
        axios.get(`http://localhost:5000/my-stats/${id}`, config).then(res => setStats(res.data));
    };

    const handleDelete = (id) => { if(window.confirm("Delete?")) axios.delete(`http://localhost:5000/products/${id}`).then(() => refreshData()); };
    ...
    const handleUpdateProfile = (e) => {
        ...
        axios.put(`http://localhost:5000/user/${user.id}`, formData).then(res => { const u = { ...user, ...res.data.user }; login(u); setShowEditModal(false); toast.success("Updated"); });
    };

    const handleOfferAction = (offerId, status) => { axios.put(`http://localhost:5000/offers/${offerId}`, { status }, { withCredentials: true }).then(() => { toast.success(`Offer ${status}`); refreshData(); }); };
    ...
    const updateStatus = (orderId, newStatus) => { axios.put(`http://localhost:5000/order-status/${orderId}`, { status: newStatus }, { withCredentials: true }).then(res => { if(res.data.Status === "Success") { toast.success(`Updated`); setMySales(mySales.map(s => s.order_id === orderId ? {...s, status: newStatus} : s)); } }); };
    const handleSubmitReview = () => { axios.post('http://localhost:5000/reviews', { seller_id: selectedSellerId, rating, comment }, { withCredentials: true }).then(res => { if(res.data.Status === "Success") { toast.success("Review Sent!"); setShowRateModal(false); } }); };

    if (!user) return <div className="text-center mt-20 font-sans">Please login first.</div>;

    const chartData = mySales.map(sale => ({ name: new Date(sale.order_date).toLocaleDateString(undefined, {month:'short', day:'numeric'}), amount: sale.price })).slice(0, 7).reverse();
        ...
        <div className="min-h-screen bg-[#FDFBF7] p-6 font-sans pt-24">
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex gap-4 mb-6 overflow-x-auto pb-2">
                    {['dashboard', 'listings', 'offers', 'messages', 'orders', 'sales', 'wishlist'].map(tab => (
                ...
                {activeTab === 'dashboard' && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm"><p className="text-xs text-stone-400 font-bold uppercase">Total Earnings</p><h3 className="text-3xl font-bold text-green-600">Rs. {stats.earnings.toLocaleString()}</h3></div>
                        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm"><p className="text-xs text-stone-400 font-bold uppercase">Total Views</p><h3 className="text-3xl font-bold text-blue-600">{stats.views}</h3></div>
                        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm"><p className="text-xs text-stone-400 font-bold uppercase">Items Sold</p><h3 className="text-3xl font-bold text-orange-600">{stats.sold}</h3></div>
                        <div className="md:col-span-2 bg-white p-6 rounded-2xl border border-stone-100 shadow-sm h-80"><h4 className="font-bold mb-4">Sales Activity</h4>{chartData.length > 0 ? (<ResponsiveContainer width="100%" height="100%"><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="name" fontSize={12} stroke="#888" /><YAxis fontSize={12} stroke="#888" /><Tooltip /><Bar dataKey="amount" fill="#EA580C" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>) : (<div className="h-full flex items-center justify-center text-stone-400 text-sm">No sales data yet.</div>)}</div>
                        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm"><h4 className="font-bold mb-4">Recent Sales</h4><div className="space-y-4">{stats.recent.map((s, i) => (<div key={i} className="flex justify-between items-center text-sm border-b pb-2"><span className="font-medium">{s.title}</span><span className="text-green-600 font-bold">+ Rs. {s.price}</span></div>))}{stats.recent.length === 0 && <p className="text-xs text-stone-400">No recent sales.</p>}</div></div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {activeTab === 'listings' && listings.map(item => (
                        <div key={item.id} className="bg-white p-4 rounded-2xl border flex flex-col gap-4 relative">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => handleDelete(item.id)} className="flex-1 bg-red-50 text-red-600 py-2 rounded-lg text-xs font-bold hover:bg-red-100"><Trash2 size={16} className="inline mr-1"/> Delete</button>
                                {item.is_sold === 0 && (<button onClick={() => handleBoost(item.id)} className={`flex-1 py-2 rounded-lg text-xs font-bold border ${item.is_featured ? 'bg-purple-100 text-purple-700 border-purple-300' : 'bg-purple-50 text-purple-600 hover:bg-purple-100 border-purple-200'}`}><Rocket size={16} className="inline mr-1"/> {item.is_featured ? "Boosted" : "Boost (Rs 50)"}</button>)}
                            </div>
                        </div>
                    ))}
                            ...
                            {conversations.map((conv, idx) => (
                                <div key={idx} className="bg-white p-4 rounded-xl border border-stone-200 flex justify-between items-center hover:shadow-md transition">
                                    <div className="flex gap-3 items-center">
                                        <div className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center font-bold text-orange-600 text-lg">{conv.other_user_name[0].toUpperCase()}</div>
                                        <div><p className="font-bold text-stone-900">{conv.other_user_name}</p><p className="text-xs text-stone-500 flex items-center gap-1"><Package size={12}/> {conv.title}</p><p className="text-[10px] text-gray-400 mt-1 flex items-center gap-1"><Clock size={10}/> {new Date(conv.last_msg_time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p></div>
                                    </div>
