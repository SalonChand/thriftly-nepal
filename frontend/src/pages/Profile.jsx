...
import { ShoppingBag, CheckCircle, Trash2, Heart, Edit2, Camera, X, Star, Package, Truck, Rocket, Gavel } from 'lucide-react';
import toast from 'react-hot-toast';
import { getImageUrl } from '../utils'; 
import { v4 as uuidv4 } from 'uuid';
import CryptoJS from 'crypto-js'; // 👈 Needed for Secure Payment
...
const Profile = () => {
    ...
    const refreshData = () => {
        ...
        axios.get(`http://localhost:5000/my-offers/${id}`, { withCredentials: true }).then(res => setOffers(res.data));
    };
    ...
    // 🚀 NEW: SECURE BOOST PAYMENT (V2)
    const handleBoost = (productId) => {
        const boostPrice = "50"; // Fixed String for hashing
        ...
        const params = { 
            amount: boostPrice, 
            tax_amount: "0", 
            total_amount: boostPrice, 
            transaction_uuid: transactionUuid, 
            product_code: productCode, 
            product_service_charge: "0", 
            product_delivery_charge: "0", 
            // Return URL with 'type=boost'
        ...
        <div className="min-h-screen bg-[#FDFBF7] p-6 font-sans pt-24">
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex gap-4 mb-6 overflow-x-auto pb-2">
                    {['listings', 'offers', 'orders', 'sales', 'wishlist'].map(tab => (
                    ...
                    {activeTab === 'listings' && listings.map(item => (
                        <div key={item.id} className="bg-white p-4 rounded-2xl border flex flex-col gap-4 relative">
                            ...
                            <div className="flex gap-2">
                                ...
                                {/* 🚀 BOOST BUTTON */}
                                {item.is_sold === 0 && (
                                    <button onClick={() => handleBoost(item.id)} className={`flex-1 py-2 rounded-lg text-xs font-bold border ${item.is_featured ? 'bg-purple-100 text-purple-700 border-purple-300' : 'bg-purple-50 text-purple-600 hover:bg-purple-100 border-purple-200'}`}>
                                        <Rocket size={16} className="inline mr-1"/> {item.is_featured ? "Boosted" : "Boost (Rs 50)"}
                                    </button>
                                )}
                            </div>
                        </div>
