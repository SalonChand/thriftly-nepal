...
const Profile = () => {
    ...
    const refreshData = () => {
        ...
        // 🔴 FIX: Define config ONCE and use it EVERYWHERE
        const config = { withCredentials: true };

        // 1. Listings
        axios.get(`http://localhost:5000/my-listings/${id}`, config)
            .then(res => setListings(Array.isArray(res.data) ? res.data : []))
            .catch(err => console.log("Listings Error:", err));

        // 2. Orders
        axios.get(`http://localhost:5000/my-orders/${id}`, config)
            .then(res => setOrders(Array.isArray(res.data) ? res.data : []))
            .catch(err => console.log("Orders Error:", err));

        // 3. Wishlist
        axios.get(`http://localhost:5000/wishlist/${id}`, config)
            .then(res => setWishlist(Array.isArray(res.data) ? res.data : []))
            .catch(err => console.log("Wishlist Error:", err));

        // 4. Sales
        axios.get(`http://localhost:5000/my-sales/${id}`, config)
            .then(res => setMySales(Array.isArray(res.data) ? res.data : []))
            .catch(err => console.log("Sales Error:", err));
        
        // 5. Offers
        axios.get(`http://localhost:5000/my-offers/${id}`, config)
            .then(res => {
                if(res.data && res.data.received) setOffers(res.data);
                else setOffers({ received: [], sent: [] });
            })
            .catch(err => console.log("Offers Error:", err));

        // 6. Conversations
        axios.get(`http://localhost:5000/my-conversations/${id}`, config)
            .then(res => setConversations(Array.isArray(res.data) ? res.data : []))
            .catch(err => console.log("Chat Error:", err));
        
        // 7. Stats
        axios.get(`http://localhost:5000/my-stats/${id}`, config)
            .then(res => setStats(res.data || { earnings: 0, sold: 0, views: 0, active: 0, recent: [] }))
            .catch(err => console.log("Stats Error:", err));
    };

    const handleDelete = (id) => { if(window.confirm("Delete?")) axios.delete(`http://localhost:5000/products/${id}`, { withCredentials: true }).then(() => refreshData()); };
    ...
    const handleUpdateProfile = (e) => { 
        ...
        axios.put(`http://localhost:5000/user/${user.id}`, formData, { withCredentials: true }).then(res => { 
    ...
    const updateStatus = (orderId, newStatus) => { 
        axios.put(`http://localhost:5000/order-status/${orderId}`, { status: newStatus }, { withCredentials: true }).then(res => { 
            if(res.data.Status === "Success") { 
                toast.success(`Updated`); 
                setMySales(mySales.map(s => s.order_id === orderId ? {...s, status: newStatus} : s)); 
            } 
        }); 
    };
    ...
    const chartData = (Array.isArray(mySales) ? mySales : []).map(sale => ({ name: new Date(sale.order_date).toLocaleDateString(undefined, {month:'short', day:'numeric'}), amount: sale.price })).slice(0, 7).reverse();
            ...
            <div className="max-w-6xl mx-auto">
                ...
                {activeTab === 'dashboard' && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm"><p className="text-xs text-stone-400 font-bold uppercase">Total Earnings</p><h3 className="text-3xl font-bold text-green-600">Rs. {stats?.earnings?.toLocaleString() || "0"}</h3></div>
                        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm"><p className="text-xs text-stone-400 font-bold uppercase">Total Views</p><h3 className="text-3xl font-bold text-blue-600">{stats?.views || "0"}</h3></div>
                        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm"><p className="text-xs text-stone-400 font-bold uppercase">Items Sold</p><h3 className="text-3xl font-bold text-orange-600">{stats?.sold || "0"}</h3></div>
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm"><h4 className="font-bold mb-4">Recent Sales</h4><div className="space-y-4">{stats.recent && stats.recent.map((s, i) => (<div key={i} className="flex justify-between items-center text-sm border-b pb-2"><span className="font-medium">{s.title}</span><span className="text-green-600 font-bold">+ Rs. {s.price}</span></div>))}{(!stats.recent || stats.recent.length === 0) && <p className="text-xs text-stone-400">No recent sales.</p>}</div></div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {activeTab === 'messages' && (<div className="col-span-full grid grid-cols-1 md:grid-cols-2 gap-4">{conversations.length === 0 && <p className="text-stone-400">No conversations.</p>}{conversations.map((conv, idx) => (<div key={idx} className="bg-white p-4 rounded-xl border border-stone-200 flex justify-between items-center hover:shadow-md transition"><div className="flex gap-3 items-center"><div className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center font-bold text-orange-600 text-lg">{conv.other_user_name[0].toUpperCase()}</div><div><p className="font-bold text-stone-900">{conv.other_user_name}</p><p className="text-xs text-stone-500 flex items-center gap-1"><Package size={12}/> {conv.title}</p><p className="text-[10px] text-gray-400 mt-1 flex items-center gap-1"><Clock size={10}/> {new Date(conv.last_msg_time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p></div></div><button onClick={() => setActiveChat(conv)} className="bg-stone-900 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-orange-600">Reply</button></div>))}</div>)}
