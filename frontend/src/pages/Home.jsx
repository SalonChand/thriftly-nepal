...
const Home = () => {
    ...
    // Updated Categories list including Art
    ...
    return (
        <div className="min-h-screen bg-[#FAFAFA] font-sans">
            ...
            <div className="bg-stone-900 text-white pt-28 pb-16 px-6 relative overflow-hidden">
                ...
                {/* 📸 THIS IS THE MISSING IMAGE LAYER 👇 */}
                ...
                <div className="max-w-4xl mx-auto text-center relative z-10">
                    <h1 className="text-4xl md:text-6xl font-serif font-bold mb-4 leading-tight">
                        Thrift<span className="text-orange-500">Ly.</span>
                    </h1>
                    ...
                    <div className="max-w-lg mx-auto relative group">
                        <input 
                            ...
                            className="w-full pl-12 pr-4 py-3.5 rounded-full text-stone-900 border-none outline-none shadow-xl focus:ring-4 focus:ring-orange-500/50 transition" 
            ...
            <div className="max-w-7xl mx-auto px-4 -mt-8 relative z-20 pb-20">
                ...
                <div className="bg-white p-5 rounded-2xl shadow-lg border border-stone-100 mb-8 flex flex-col gap-4 relative">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                        {categories.map(cat => (
                            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-1.5 rounded-full font-bold text-sm border whitespace-nowrap transition ${selectedCategory === cat ? 'bg-stone-900 text-white border-stone-900' : 'bg-stone-50 text-stone-600 border-stone-100 hover:border-stone-300'}`}>{cat}</button>
                        ))}
                    </div>
                    ...
                    <div className="border-t border-stone-100 pt-4 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div className="flex flex-wrap gap-2 w-full md:w-auto items-center">
                            ...
                            <select onChange={(e) => setSelectedSize(e.target.value)} className="px-3 py-2 rounded-lg border border-stone-200 bg-white text-stone-700 text-sm font-bold outline-none cursor-pointer hover:border-stone-400">
                                <option value="All">Size: All</option><option value="S">S</option><option value="M">M</option><option value="L">L</option><option value="XL">XL</option>
                            </select>
                            <select onChange={(e) => setSelectedCondition(e.target.value)} className="px-3 py-2 rounded-lg border border-stone-200 bg-white text-stone-700 text-sm font-bold outline-none cursor-pointer hover:border-stone-400">
                                <option value="All">Condition: All</option><option value="Good">Good</option><option value="New">New</option><option value="Like New">Like New</option><option value="Fair">Fair</option>
                            </select>
                        </div>

                        <div className="flex items-center gap-2 w-full md:w-auto">
                            <ArrowUpDown size={16} className="text-stone-400" />
                            <select onChange={(e) => setSortOption(e.target.value)} className="px-3 py-2 rounded-lg border border-stone-200 bg-stone-50 text-stone-900 text-sm font-bold outline-none cursor-pointer hover:border-orange-500 transition w-full md:w-auto">
                ...
                {filteredProducts.length === 0 && (
                    <div className="text-center text-stone-400 py-24">
                        ...
                        <button onClick={() => {setSelectedCategory("All"); setSelectedSize("All"); setSelectedCondition("All"); setSearchTerm("")}} className="mt-4 text-orange-600 font-bold hover:underline">Clear filters</button>
                    </div>
                )}
            </div>
        </div>
