...
const Home = () => {
    ...
    const toggleWishlist = (e, productId) => {
        ...
        axios.post('http://localhost:5000/wishlist/toggle', { user_id: user.id, product_id: productId }, { withCredentials: true })
            .then(res => { 
                if(res.data.Status === "Added") { setWishlistIds([...wishlistIds, productId]); toast.success("Added to Wishlist"); } 
                else { setWishlistIds(wishlistIds.filter(id => id !== productId)); toast.success("Removed from Wishlist"); }
            });
    };
