...
import Stories from './pages/Stories'; // 📸 ADDED THIS
...
const Navbar = () => {
  ...
  const handleShowNotifications = () => {
      if (!showNotif) {
          ...
          axios.put('http://localhost:5000/notifications/read', {}, { withCredentials: true }).catch(err => console.log("Error marking read"));
      }
    ...
    <nav className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-stone-100 transition-all duration-300">
      <div className="container mx-auto px-6 h-20 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="text-3xl font-serif font-bold text-stone-900 tracking-tighter">Thrift<span className="text-orange-600">Ly.</span></span>
        </Link>
        <div className="hidden md:flex items-center gap-8">
            <Link to="/" className="text-stone-500 hover:text-stone-900 font-medium transition">Shop</Link>
            <Link to="/stories" className="text-stone-500 hover:text-stone-900 font-medium transition">Stories</Link> {/* 📸 ADDED LINK */}
                    ...
                    <div className="relative">
                        ...
                        {showNotif && (
                            <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-stone-100 overflow-hidden animate-fade-in z-50">
                                <div className="p-3 font-bold border-b text-sm flex justify-between items-center bg-stone-50"><span>Notifications</span><button onClick={() => setShowNotif(false)}><X size={14}/></button></div>
                                <div className="max-h-80 overflow-y-auto">{notifications.length === 0 ? <p className="p-4 text-xs text-stone-400 text-center">No new notifications</p> : notifications.map((n, i) => (<button key={i} onClick={() => handleNotifClick(n)} className={`w-full text-left p-3 text-xs border-b hover:bg-stone-50 flex flex-col gap-1 transition bg-white`}><span className="font-medium text-stone-800">{n.message}</span><span className="text-[10px] text-stone-400 flex items-center gap-1"><Clock size={10}/> {new Date(n.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span></button>))}</div>
                            </div>
                        )}
                    </div>
                    <Link to="/profile" className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center overflow-hidden border border-orange-200 hover:bg-orange-200 transition">{user.profile_pic ? <img src={getImageUrl(user.profile_pic)} className="w-full h-full object-cover" /> : <span className="text-orange-600 font-bold">{user.username ? user.username[0].toUpperCase() : <User size={18}/>}</span>}</Link>
      ...
      {isOpen && (<div className="md:hidden bg-white border-t border-stone-100 p-6 flex flex-col gap-4 shadow-xl"><Link to="/" className="text-lg font-bold text-stone-700" onClick={() => setIsOpen(false)}>Shop</Link><Link to="/stories" className="text-lg font-bold text-stone-700" onClick={() => setIsOpen(false)}>Stories</Link><Link to="/sell" className="text-lg font-bold text-stone-700" onClick={() => setIsOpen(false)}>Sell Items</Link><Link to="/about" className="text-lg font-bold text-stone-700" onClick={() => setIsOpen(false)}>About</Link><Link to="/contact" className="text-lg font-bold text-stone-700" onClick={() => setIsOpen(false)}>Contact</Link><div className="border-t border-stone-100 my-2"></div>{user ? (<><Link to="/profile" className="text-lg font-bold text-orange-600" onClick={() => setIsOpen(false)}>My Profile</Link><button onClick={() => { logout(); setIsOpen(false); }} className="text-lg font-bold text-stone-400 text-left">Logout</button></>) : (<><Link to="/login" className="text-lg font-bold text-stone-700" onClick={() => setIsOpen(false)}>Login</Link></>)}</div>)}
    </nav>
  );
};
        ...
        <div className="min-h-screen flex flex-col font-sans bg-white">
          ...
          <div className="flex-grow pt-20"> 
            <Routes>
              ...
              <Route path="/stories" element={<Stories />} /> {/* 📸 ADDED ROUTE */}
